...
// Rendering-safe posterior chunks: every chunk is valid TeX by itself.
...
// Parameter-estimation formulas missing from the official sheet.
...
// PAC: realizability is distinct from Version Space.
...
// Regression and linear classification details.
...
// SVM: full context for every KKT/Lagrangian question.
...
const kkt=B.items.find(x=>x.id==='svm_kkt');if(kkt){kkt.fullContext={intro:'Complementary slackness is one KKT condition. Read it together with the hard-margin primal, Lagrangian and stationarity conditions.',steps:[
 ...
 {label:'Constraint in g_i\u22650 form',tex:String.raw`g_i(w,b)=y_i(w^Tx_i-b)-1\ge0`},
...
// Boosting / backprop / CNN / optimizer formulas absent from the official sheet.
...
// PCA / GMM formulas missing from the previous bank.
